...
def bpca_fill(dX, max_iter=200, verbose=False):
    y = pd.DataFrame(
        dX
    )  # Just following the conventions used in the original paper by Oba et al. (2003)
    ...
    q = d - 1
    yest = np.copy(y)
    yest[np.isnan(yest)] = 0

    missidx = (
        {}
    )  # dictionary that contains NaN positions (row,col) of dX; {row : list of NaN col in row}
    bad = np.where(np.isnan(y))
    badrows = bad[0]
    badcols = bad[1]
    for row in badrows:
        missidx[row] = []
    for link in range(len(badrows)):
        missidx[badrows[link]].append(badcols[link])
    ...
    for row in goodrows:
        nomissidx[row] = []
    for link in range(len(goodrows)):
        nomissidx[goodrows[link]].append(goodcols[link])
    ...
    taumax = 1e20
    taumin = 1e-20
    tau = 1 / (np.trace(covy) - np.trace(S))
    tau = np.amax([np.amin([tau, taumax]), taumin])

    galpha0 = 1e-10
    balpha0 = 1
    alpha = (2 * galpha0 + d) / (tau * np.diag(W.T * W) + 2 * galpha0 / balpha0)
    gmu0 = 0.001
    btau0 = 1
    gtau0 = 1e-10
    SigW = np.eye(q)
    ...
    for epoch in range(max_iter):
        ...
        Rx = np.eye(q) + tau * W.T * W + SigW
        Rxinv = np.linalg.inv(Rx)
        idx = gnomiss
        n = len(idx)
        dy = y[idx, :] - np.tile(mu, (n, 1))
        x = tau * Rxinv * W.T * dy.T

        Td = dy.T * x.T
        trS = np.sum(np.multiply(dy, dy))
        for n in range(len(gmiss)):
            i = gmiss[n]
            dyo = np.copy(y)[i, nomissidx[i]] - mu[nomissidx[i]]
            Wm = W[missidx[i], :]
            Wo = W[nomissidx[i], :]
            Rxinv = np.linalg.inv(Rx - tau * Wm.T * Wm)
            ex = tau * Wo.T * np.matrix(dyo).T
            x = Rxinv * ex
            dym = Wm * x
            dy = np.copy(y)[i, :]
            dy[nomissidx[i]] = dyo
            dy[missidx[i]] = dym.T
            yest[i, :] = dy + mu
            Td = Td + np.matrix(dy).T * x.T
            Td[missidx[i], :] = Td[missidx[i], :] + Wm * Rxinv
            trS = (
                trS
                + dy * np.matrix(dy).T
                + len(missidx[i]) / tau
                + np.trace(Wm * Rxinv * Wm.T)
            )
        ...
        tau = (d + 2 * gtau0 / N) / (
            trS
            - np.trace(Td.T * W)
            + (mu * np.matrix(mu).T * gmu0 + 2 * gtau0 / btau0) / N
        )[0, 0]
        SigW = Dwinv * (d / N)
        alpha = (2 * galpha0 + d) / (
            tau * np.diag(W.T * W) + np.diag(SigW) + 2 * galpha0 / balpha0
        )

        dtau = np.log10(tau) - np.log10(tau_old)
        if np.abs(dtau) < 1e-4:
            if verbose:
                print(f"Stopping at iteration {epoch+1}")
            break
