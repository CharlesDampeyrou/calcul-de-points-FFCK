...
from points_methods.tensorial_product_estimator import (
    estimate_tensorial_product,
    improve_vectors,
    improve_competitor_vector,
...
def find_minimal_iteration_first_optim(scores_df, participations_df):
    ...
    for first_iterations in range(5, 51, 5):
        ...
        _, _ = estimate_tensorial_product(
            scores_df,
        ...
        print(
            ...
            % (
                nb_differences,
                nb_differences / participations_df_copy.sum().sum(),
                first_iterations,
            )
        )


def find_minimal_iterations(
    scores_df, participations_df, iterations_first_optimization=50, removing_rate=0.2
):
    participation_df_copy = participations_df.copy()
    target_competition_vector, target_competitor_vector = estimate_tensorial_product(
        scores_df, participation_df_copy
    )
...
if __name__ == "__main__":
    ...
    pc = PointsComputer(point_type, value_accessor, db_service)
    nb_tests = 20
