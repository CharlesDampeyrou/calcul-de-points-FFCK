...
def estimate_tensorial_product(
    scores_df,
    participations_df,
    ...
    competitor_vector = improve_competitor_vector(
        scores_df, participations_df, competition_vector
    )
    to_remove = int(removing_rate * participations_df.sum().sum())
    already_removed = 0
    for i in range(iterations_first_optimization):
        (competition_vector, competitor_vector) = improve_vectors(
            scores_df, participations_df, competition_vector, competitor_vector
        )
        to_remove_on_iteration = (
            int((i + 1) / iterations_first_optimization * to_remove) - already_removed
        )
        remove_underperformances(
            scores_df,
            participations_df,
            competition_vector,
            competitor_vector,
...
def improve_vectors(
    ...
    new_competition_vector = improve_competition_vector(
        scores_df, participations_df, competitor_vector
    )
    new_competition_vector *= (
        new_competition_vector.shape[0] / new_competition_vector.sum()
    )
    new_competitor_vector = improve_competitor_vector(
        scores_df, participations_df, new_competition_vector
    )
    return new_competition_vector, new_competitor_vector


def improve_competition_vector(scores_df, participations_df, competitor_vector):
...
def improve_competitor_vector(scores_df, participations_df, competition_vector):
...
def remove_underperformances(
    scores_df, participations_df, competition_vector, competitor_vector, nb_to_remove
):
    deviation = np.array(scores_df) - np.tensordot(
        competition_vector, competitor_vector, axes=0
    )
    deviation *= np.array(participations_df)
    to_remove = (
        deviation.argsort(axis=None)[-nb_to_remove:] if nb_to_remove != 0 else []
    )
