...
class PointsComputer:
    ...
    def compute_and_save_points(self, **kwargs):
        ...
        for competition_name in competition_names:
            self.logger.debug(
                "Calcul et sauvegarde des points de la course %s", competition_name
            )
    ...
    def initialize(self, origin_points="scrapping", **kwargs):
        ...
        self.database_service.copy_points(
            starting_date, date, self.point_type, origin_points
        )
    ...
    def compute_points(self, participations):
        ...
        coef_correcteur, ratio_points_verts = self.calcul_coef_correcteur(
            participations
        )
        ...
        metadata = {
            "competitionName": participations[0]["competitionName"],
            "date": participations[0]["date"],
            "simplifiedCompetitionPhase": participations[0][
    ...
    def calcul_tps_base(self, participations):
        ...
        meilleurs_participations = self.get_meilleurs_participations(
            participations, valeur_max
        )
        if len(meilleurs_participations) < 5:
            ...
            raise NotEnoughCompetitorException(
                msg
                % (
                    participations[0]["competitionName"],
    ...
    def get_meilleurs_participations(self, participations, valeur_max):
        ...
        for participation in participations:
            if (
                participation["values"][self.value_type]["points"] < valeur_max
            ):  # TODO : vrai comparaison de valeurs ?
