...
class CompetitionProcessor:
    def __init__(self, database_service, point_computer):
    ...
    def initialize_point_type(self, date, origin_points="scrapping"):
        self.logger.info(
            "Starting the initialisation of the point type %s..."
            % self.point_computer.point_type
        )
        self.point_computer.initialize(date=date, origin_points=origin_points)
        self.logger.info(
            "Initialisation of the point type %s finished"
            % self.point_computer.point_type
        )

    def compute_point_type(self, starting_date):
        self.logger.info(
            "Starting to cumpute the %s points..." % self.point_computer.point_type
        )
        competition_dates = self.db_service.get_competition_dates(starting_date)
        ...
        self.logger.info(
            msg
            % (
                len(competition_dates),
                str(min(competition_dates)),
                str(max(competition_dates)),
            )
        )
        ...
        for competition_date in competition_dates:
            ...
            for phase in phases:
                competition_names = list(
                    self.db_service.get_competition_list(competition_date, phase=phase)
                )
                if (
                    competition_names
                ):  # La méthode n'est pas appelée si la liste est vide
                    self.point_computer.compute_and_save_points(
