...
class Scrappeur(object) :
    def __init__(self) :
        ...
        self.phases = list()
        self.logger = logging.getLogger("data_handling.Scrappeur")
    ...
    def save_all_files(self, rep=Path(Path.home(), "database_courses_ffck")):
    ...
    def save_competition(rep, titre, event_name, niveau, phase, names_list, emb_list,
        ...
        event_name_modif = event_name.replace('/', ' sur ')
        phase_modif = phase.replace("1/2", "demi")
        file_name = date + "-" + event_name_modif + "-" + phase_modif + ".csv"
        file_name = replace_chars(file_name, ["/", "\"", "\\", "|"], "-")
        file_path = Path(rep, file_name)
