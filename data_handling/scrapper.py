...
class Scrappeur(object):
    ...
    async def save_all_competitions(self, first_saving=False):
        ...
        for i in range(0, len(self.competitions_url), subset_size):
            msg = "Récupération et sauvegarde des compétitions %s à %s"
            self.logger.info(msg % (i, i + subset_size))
            subset_urls = self.competitions_url[i : i + subset_size]
            competition_infos_list = await asyncio.gather(
                *(get_competition_infos(url) for url in subset_urls)
            )
            for j in range(len(competition_infos_list)):
                niv = self.niveaux[i + j]
                phase = self.phases[i + j]
                event = self.event_names[i + j]
...
class ClasseTest:
    ...
    async def get_page(self, req_id):
        async with aiohttp.ClientSession() as session:
            ...
            url = (
                "http://www.ffcanoe.asso.fr/eau_vive/slalom/classement/evenements/index"
            )
            async with session.get(url) as response:
                content = await response.content.read()
        print(
            "Requête effectuée (id:%d), type de content : %s" % (req_id, type(content))
        )
        return "done"
